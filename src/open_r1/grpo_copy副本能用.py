...
class GRPOScriptArguments(ScriptArguments):  
    ...
    reward_funcs: list[str] = field(
        ...
        metadata={
            "help": "List of reward functions. Possible values: 'accuracy', 'format', 'length'"
        },
    )
    ...
    #     default_factory=lambda: [0.8, 0.15, 0.05],
    #     metadata={
    #         "help": "Weights for each reward function (must match reward_funcs order and length)"
    #     },
    # )
...
def main(script_args, training_args, model_args,callbacks):  
    ...
    for func, weight in zip([REWARD_FUNCS_REGISTRY[func] for func in script_args.reward_funcs], training_args.reward_weights):
        # 创建带权重的奖励函数
        def weighted_reward(completions, solution, _func=func, _weight=weight, _name=func.__name__, **kwargs):
            # 检查函数是否需要solution参数
            if _name == "format_reward":
                # format_reward不需要solution参数
                rewards = _func(completions, **kwargs)
            else:
                # 其他奖励函数需要solution参数
                rewards = _func(completions, solution, **kwargs)
            return [r * _weight for r in rewards]
